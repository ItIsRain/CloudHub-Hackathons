...
export default function RegisterPage() {
  ...
  // Only render the form fields needed for the current step
  ...
  // Optimize account type rendering - only change if selection changes
  const renderAccountTypeOptions = useMemo(() => {
  ...
  }, [formData.accountType, isFirstStep]);
  ...
  // Optimize the progress stepper - only re-render when currentStep changes
  ...
  // Optimize rendering of personal info step
  ...
  }, [isSecondStep, formData.name, formData.email, formData.password, formData.confirmPassword, formData.phoneNumber, formData.countryCode, formData.country, formData.nationality, formData.acceptTerms, showPassword, showConfirmPassword]);

  // Complete step rendering is memoized
  const renderCompleteStep = useMemo(() => {
  ...
  }, [isLastStep, formData.accountType, formData.email]);
  ...
  }, [currentStep, isLoading, formData.acceptTerms, formData.accountType, formData.name, formData.email, formData.password, formData.confirmPassword, handlePreviousStep, handleNextStep]);
  ...
  return (
    <div className="min-h-screen w-full relative flex items-center justify-center bg-gradient-to-b from-slate-50 to-blue-50 dark:from-slate-950 dark:to-slate-900 overflow-hidden">
      {/* Modern decorative elements - these don't change often so no need to optimize */}
          ...
          <Card className="border-none shadow-xl w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-md mx-auto max-w-3xl overflow-hidden rounded-2xl">
            <CardContent className="pt-6 pb-4 px-6 sm:px-8">
              <form onSubmit={handleSubmit}>
                ...
                {/* Profile Details Step - only render if on that step */}
                {isThirdStep && (
                  <div className="space-y-4 py-2 max-w-5xl mx-auto">
                    <div className="text-center mb-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      {/* Rest of profile step content - see the original code */}
                      {/* For brevity, I'm not including all fields here, but they would remain the same */}
                    </div>
                  </div>
                )}
